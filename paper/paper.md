...
The library provides scalar and batch interfaces for serialized sequences and parsed ProForma annotations. Batch results can be assigned to tabular data, including pandas DataFrames [@team-2025]. Streaming input and optional error collection support workflows in which some ProForma annotations are unresolved or unsuitable for a requested operation.

The aim is to pass one parsed ProForma annotation through editing, digestion, and chemical analysis without losing represented information. Released source tests check ProForma annotation round trips, numerical agreement with Pyteomics, and BRAIN isotope envelopes against direct convolution. Analysis of cross-linked and branched peptidoforms is out of scope.

# State of the Field

Existing packages address overlapping needs. **Pyteomics** [@goloborodko-2013; @levitsky-2019] supports ProForma parsing, mass and composition calculations, and fragment generation alongside its historical modX format [@pyteomics-docs]. **Biopython** [@cock-2009] provides general sequence analysis, including protein property calculations through its ProtParam module [@biopython-protparam]. The **mzcore** and related RustyMS libraries provide ProForma support, chemical representations, and theoretical fragmentation, with Python bindings for selected components [@Schulte_mzcore; @schulte-2025]. **pyOpenMS** [@rost-2014] exposes a broader mass spectrometry toolkit. Current OpenMS documentation also describes ProForma parsing and conversion [@openms-proforma].

Peptacular's contribution is a common, editable ProForma annotation model shared by sequence transformations and chemical analysis in Python. This design allows researchers to inspect and modify ProForma annotations directly while using consistent charge, modification, and ambiguity handling across operations. Peptacular is written in pure Python, so it installs without a compiler toolchain and supports free-threaded builds. A dedicated package keeps this interface focused on sequence analysis. Optional adapters convert ProForma annotations to the objects used by Pyteomics, psm_utils [@gabriels-2023], and AlphaBase, with checks for conversion losses. These integrations complement the specialized capabilities of the surrounding ecosystem.
