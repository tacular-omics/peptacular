...
- Fragmentation of cross-linked peptidoform ions via `MultiProFormaAnnotation.fragment()`, controlled by a chemistry-agnostic `cross_link_mode` setting (`CrossLinkFragmentMode`, exported from `peptacular`): `"cleavable"` fragments each chain independently, `"non_cleavable"` makes a link-spanning backbone fragment carry the intact partner chain(s) (and suppresses fragments that would sever an intra-chain loop), and `"both"` emits the union. All masses come from the cross-linker mass peptacular already computes — no per-linker chemistry tables. Each chain is fragmented with the existing single-peptide engine; returned `Fragment` objects gain `chain_index` and `cross_link_series` (`"linear"`/`"cleavable"`/`"non_cleavable"`) provenance fields. Only forward/backward backbone ions are supported for cross-linked ions (internal/immonium ions raise).
