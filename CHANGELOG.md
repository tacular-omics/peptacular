...
### Fixed
- `Fragment.to_mzpaf` wrote a formula gain (`deltas={"H2O": -1}`) as a loss (`-H2O`); it now writes `+H2O`. A numeric mass delta is written as a signed mass (`b2-34.0` for `{-17.0: 2}`) instead of raising.
