...
### Fixed
- Loss and isotope ions of peptides with named modifications keep the mods' listed database masses. With `deltas=` (formula losses such as `"H2O"`), `neutral_deltas=`, `isotopes=` or a deprotonating charge, `frag()`, `fragment()` and `mass()` summed the mods' elemental compositions instead, so `PEM[Oxidation]TIDEK` y6-H2O was 3.8e-7 Da off the plain y6 minus water. A loss or isotope peak is now exactly the plain ion plus its delta. Composition is still used for `calculate_with_composition=True` and under global isotope labels (`<13C>`).
