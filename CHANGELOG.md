...
- `fragment()` raised `InvalidAdjustmentError` for a whole d/w/v series when one ion could not exist, such as the one-residue d1 of `V-[Amidated]` at charge -1. The series now leaves that ion out; an explicit `frag()` of it still raises.
