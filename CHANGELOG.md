...
- `ChargedFormula.to_mz_paf` now puts the ordinal count before the atom (e.g. `2H`) per mzPAF section 4.5, instead of reusing ProForma's own formula notation (`H2`); this affects mzPAF serialization of any fragment neutral loss or charge adduct with an element count greater than 1
